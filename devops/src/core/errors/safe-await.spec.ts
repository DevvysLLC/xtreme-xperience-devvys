...
test('given rejected promise with a string error', async (t) => {
  ...
  t.true(err instanceof DevOpsError)

  if (err instanceof DevOpsError === false) {
    throw new Error('should have DevOpsError')
  }

  t.is(err.message, 'Error: Test error')
  t.is(err.details.err, message)
  t.is(data, null)
})
...
test('given rejected promise with a non-Error error', async (t) => {
  ...
  t.is(err.details.err, originalError)
  t.is(data, null)
})
