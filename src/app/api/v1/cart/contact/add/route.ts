...
const logger = initLogger().child({ name: 'cart-contact-add-api' })
...
export const POST = async (request: Request): Promise<NextResponse> => {
  try {
    ...
    const result = await cartService.addContacts(parsed.data, cartKey, userGuid)
    ...
    logger.error(error, 'Internal error processing cart contact add request')
    return NextResponse.json(
      {
        status: 'error',
