...
export class CartService {
  ...
  async processCart(
    ...
    const url = `${this.baseUrl}${ROUTES.ROCKET_REZ.CART.PROCESS}`
    ...
    logger.info(
      ...
      'rocket-rez.cart-service.processCart'
    )
    ...
    if (error) {
      throw new AppError(
        error.message ?? 'Unknown error occurred while processing cart',
        {
          traceTag: 'rocket-rez.cart-service.processCart',
          cause: error
        }
      )
    }
    ...
    if (!response.ok) {
      ...
      throw new AppError('RocketRez API error during cart process', {
        traceTag: 'rocket-rez.cart-service.processCart',
